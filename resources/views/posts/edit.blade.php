...
<body class="min-h-screen bg-slate-100 text-gray-900 antialiased">
    <div class="mx-auto max-w-3xl px-4 py-10 sm:px-6 lg:px-8">
        <header class="mb-8">
            ...
            <h1 class="mt-4 text-3xl font-semibold tracking-tight text-slate-900">Edit Post</h1>
        ...
        <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
            ...
            <form action="{{ route('post.update', $edit_post->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <input type="text" id="title" name="title" required
                        value="{{ old('title', $edit_post->title) }}"
                ...
                <div>
                    ...
                    <textarea id="body" name="body" rows="9" required
                        class="w-full rounded-xl border bg-white px-4 py-3 text-sm leading-7 text-slate-900 outline-none transition focus:ring-2 {{ $errors->has('body') ? 'border-red-500 focus:border-red-500 focus:ring-red-100' : 'border-slate-300 focus:border-slate-900 focus:ring-slate-200' }}">{{ old('body', $edit_post->body) }}</textarea>
                    @error('body')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
